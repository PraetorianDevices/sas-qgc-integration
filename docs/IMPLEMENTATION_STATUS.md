...
**Overall Status:** Phases 0, 1, 1.5, and 2 are complete. All seven bridges are verified against pymavlink, mission signing is wired into the live upload path, secure_launch.py is fixed, and the entire mavlink-bridge test suite (201 tests) imports and exercises real production code. Phase 2 (Fleet Manager / Collision / Emergency Wipe bridges) is now done, and the inbound single-UDP-port limitation it introduced is resolved via `mavlink_router_node.py`. Phase 3 (QGC plugin) remains not started. Live validation is in progress: both packages now colcon-build cleanly and import against real `px4_msgs` (not test stubs) for the first time in this project — see "Live ROS 2 Build & Validation" below — though nothing has been launched/run yet.
...
**Full mavlink-bridge suite: 201 tests passed, zero exclusions** (previously, `test_mavlink_crc.py` had to be excluded because it required a real ROS 2 environment; that's no longer true). Full SAS unit suite: 1215 passed, 3 skipped — unaffected by this work.
...
## Part 5: Live ROS 2 Build & Validation — IN PROGRESS (build/import verified, nothing launched yet)

The first real ROS 2 environment used in this whole project (WSL Ubuntu 24.04, ROS 2 Jazzy, `colcon`) surfaced several real bugs that no amount of unit/integration testing against stubs could have caught, since the stubs supplied exactly the attributes the code expected rather than modeling what real ROS 2 tooling and real PX4 messages actually look like.

**Environment discovered already partially set up** (not created this session): `~/PX4-Autopilot` was already cloned and PX4 SITL already built (`build/px4_sitl_default/bin/px4`, valid ELF binary, built 2026-06-28); a colcon workspace at `~/ros2_ws` already had `my_python_package` (SAS) built from 2026-04-02; Gazebo Harmonic was already installed. None of this was rebuilt or altered beyond adding `mavlink-bridge` and `px4_msgs` to the same workspace. QGroundControl is separately confirmed installed as a native Windows application (`C:\Program Files\QGroundControl\bin\QGroundControl.exe`, ~1900 files, full Qt/GStreamer runtime) — not launched.

**Packaging bugs found and fixed while getting `colcon build` to succeed at all** (none of these were caught by 201 passing unit/integration tests, since none of them exercise packaging or `ros2 launch`/`ros2 run` resolution):
1. **`package='SAS'` in three launch files** (`launch_sas_qgc_integration.py`, `launch_gps_spoof_qgc.py`, `tests/integration/launch_full_integration.py`) — the actual registered ROS 2 package name is `my_python_package` (per `SAS/package.xml` and `SAS/setup.py`); `SAS` is only the repo/directory name, never a real package. Would fail with `package 'SAS' not found` regardless of build state. Fixed to `package='my_python_package'` in all three.
2. **`mavlink-bridge/package.xml` missing `<export><build_type>ament_python</build_type></export>`** — without it, `colcon build` couldn't tell the package was Python and tried (and failed) to configure it as a CMake package. Added the export block (matching `SAS/package.xml`'s existing convention) and `<depend>std_srvs</depend>` (used by `emergency_wipe_mavlink_bridge.py`, previously undeclared).
3. **`mavlink-bridge/setup.py` missing the standard `data_files` block** (ament_index resource_index registration + launch file installation) — without it, colcon installs the package with no ament_index marker and no launch files copied to `share/`, so neither `ros2 pkg list` nor `ros2 launch mavlink-bridge <file>.py` could find it post-install. Added the block (plus a new `resource/mavlink-bridge` marker file), installing the two top-level launch files by name into `share/mavlink-bridge/launch/` without moving them in the source tree (avoids touching the many existing doc references to their current paths).
4. **`mavlink-bridge` missing `setup.cfg`** — without the `[develop]`/`[install]` `script_dir`/`install_scripts` override every other ament_python package in this repo has, setuptools installed all 8 console_scripts entries into a generic `bin/` directory instead of ROS 2's expected `lib/mavlink-bridge/`, so `ros2 run`/`ros2 pkg executables mavlink-bridge` found zero executables even though the build "succeeded." Added `setup.cfg` matching `SAS/setup.cfg`'s convention.

**Result:** `colcon build --symlink-install --packages-select my_python_package mavlink-bridge` succeeds cleanly; `ros2 pkg executables mavlink-bridge` correctly lists all 8 executables; `ros2 launch mavlink-bridge launch_sas_qgc_integration.py --show-args` resolves the launch file **by package name** (not just raw file path) for the first time in this project.

**`px4_msgs` cloned and built** (was entirely missing from the environment — needed at runtime by `telemetry_mavlink_bridge.py`, `collision_mavlink_bridge.py`, and several SAS nodes; doesn't block a *build*, since Python doesn't check attribute existence until an attribute is actually accessed, but does block anything actually *running*). Cloning it and importing the real messages for the first time (instead of test stubs) is what surfaced the `telemetry_mavlink_bridge.py` field bugs documented in Part 1's bug list above — a second-order benefit of getting the real build working, not something a code review of the bridge in isolation would have found.

**Not yet done:** nothing has been launched — no node started, no PX4 SITL run, no QGroundControl opened, no `ros2 launch` beyond `--show-args`. That's the next step (see Immediate Next Steps).
...
- ✅ Full mavlink-bridge test suite (201 tests) and SAS unit suite (1215 tests) — all importing and exercising real code.
...
- ✅ QGroundControl confirmed installed as a native Windows application; PX4 SITL confirmed already built in the WSL environment (from an earlier, unrelated session) — neither has been launched yet.

## What Is NOT Ready

- ⏳ QGC Custom Plugin (Phase 3) — not started, requires C++/Qt/QML.
- ⏳ Live ROS 2/WSL validation — the packages now build and import cleanly against real ROS 2/px4_msgs (Part 5), but nothing has actually been launched: no bridge node started, no PX4 SITL run, no QGroundControl opened. Everything's correctness claims to date are unit/integration-level (pymavlink byte comparison, real sockets, real crypto, real build tooling) — none of it has been exercised against the live external systems it's meant to interoperate with.

---

## Immediate Next Steps (Priority Order)

1. **Actually launch the stack.** With both packages now colcon-built, the next step is running `ros2 launch mavlink-bridge launch_sas_qgc_integration.py` for real (with PX4 SITL, already built, providing telemetry, and real QGroundControl, already installed, as the far end) — the first genuine end-to-end run this project has had. This is also the first opportunity to confirm `enable_security:=true` actually starts nodes under DDS-Security enforcement rather than just setting env vars correctly (verified so far only that far).
...
3. **No live validation yet:** both packages now colcon-build and import cleanly against real ROS 2/px4_msgs (Part 5), but nothing has been launched — no node started, no PX4 SITL run, no QGroundControl opened. `enable_security:=true` is confirmed to set the right env vars and log the right message, but never run through to nodes actually starting under DDS-Security enforcement.
